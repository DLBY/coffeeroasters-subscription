    ...
    <section className="container mx-auto px-6">
      <div>
        ...
        <div className="grid grid-cols-1 justify-items-center gap-8 md:gap-[30px] xl:grid-cols-4 mx-auto w-[80%]" >
          {collection.map((item) => (
            <div className='flex flex-col md:flex-row xl:flex-col items-center xl:justify-center text-center md:text-left first:mt-0 mt-12 md:mt-0 md:gap-[36px]' key={item.id}>
              <img
                src={item.image}
                alt={item.name}
              ...
              <div className="flex flex-col mt-6 md:mt-0 xl:text-center">
                <h3 className="font-serif text-darkGreyBlue text-2xl">{item.name}</h3>
                <p className="text-base text-darkGreyBlue mt-4 md:mt-[24px] xl:mt-6">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
